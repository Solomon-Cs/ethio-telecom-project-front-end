...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    await registerUser.mutateAsync(formData).then(() => {
      toast({
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[#f8fafc] dark:bg-slate-950 p-4 py-12">
      ...
      <div className="w-full max-w-[550px] space-y-8 relative z-10">
        <div className="text-center space-y-2">
          <Link href="/auth/login" className="inline-flex items-center text-xs text-muted-foreground hover:text-primary transition-colors mb-4 font-bold uppercase tracking-widest">
            <ArrowLeft className="mr-2 h-3 w-3" /> Back to Login
          </Link>
          <h1 className="text-3xl font-black tracking-tight">Start Your Journey</h1>
          <p className="text-muted-foreground font-medium">Join thousands of smart spenders today</p>
        </div>
        ...
        <Card className="border-none shadow-2xl rounded-3xl overflow-hidden glass-morphism">
          ...
          <CardContent className="space-y-6">
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <Button type="submit" className="w-full rounded-xl h-11 font-bold shadow-lg shadow-primary/20 mt-4" disabled={isLoading}>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-card px-2 text-muted-foreground font-bold">Fast sign up with</span>
              </div>
            </div>

            <Button variant="outline" onClick={handleGoogleLogin} className="w-full rounded-xl h-11 border-border/50 hover:bg-muted/50 transition-all font-semibold">
