...
import { Transaction, UserProfile, FinancialSummary, TransactionCategory } from '@/types/finance';
...
export function useFinanceStore() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        setProfile(parsed.profile || { username: 'Financial Maven', email: 'maven@finance.com', currency: '$' });
      } catch (e) {
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ transactions, profile }));
    }
