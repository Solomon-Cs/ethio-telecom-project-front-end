...
export function useCategories(userId?: string, filters: TransactionFilters = {}) {
    ...
    const { data: categoriesByUser, isLoading: isLoadingByUser, error: errorByUser, refetch: refetchByUser } = useQuery({
        queryKey: ['categoriesByUser', filters],
        queryFn: () => categoryService.getCategoryByUserId(userId as string, filters),
    });
    console.log("🚀 ~ useCategories ~ categoriesByUser:", categoriesByUser)
