...
export function SummaryCards({ summary }: { summary: FinancialSummary }) {
  ...
  const getTopCategory = (
    ...
    if (!summaryByCategoryDate?.length) return null;

    // pick key based on type
    const key = type === 'INCOME' ? 'totalIncomes' : 'totalExpenses';

    // find top category by that key
    const top = summaryByCategoryDate.reduce((max: any, current: any) => {
      return (current[key] ?? 0) > (max[key] ?? 0) ? current : max;
    }, summaryByCategoryDate[0]);

    // If top amount is 0, return null
    if ((top[key] ?? 0) === 0) return null;
  ...
  console.log('🚀 Top Income:', topIncome);

  const topExpense = getTopCategory('EXPENSE');
  console.log('🚀 Top Expense:', topExpense);
