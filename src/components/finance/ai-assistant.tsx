...
import { financialAssistantQuery } from '@/ai/flows/financial-assistant-query';
...
export function AIAssistant() {
  ...
  const handleAsk = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: "I encountered an error while analyzing your data. Please try again." }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Card className="h-[600px] flex flex-col border-none shadow-2xl overflow-hidden glass-morphism rounded-3xl">
            ...
            {messages.length === 0 && (
              <div className="text-center py-12 space-y-4">
                ...
                <div className="space-y-1">
                  <p className="font-bold text-sm">Hello! I'm your Finance AI.</p>
                  <p className="text-xs text-muted-foreground px-8">Ask me about your top expenses, monthly trends, or budget status.</p>
                </div>
                <div className="flex flex-wrap justify-center gap-2 pt-4">
                  {["Top expenses?", "Food budget?", "Monthly trend?"].map(hint => (
                    <Button 
                      key={hint} 
                      variant="outline" 
                      size="sm" 
            ...
            {isLoading && (
              <div className="flex gap-3">
                ...
                <div className="bg-card border border-border/50 p-4 rounded-2xl rounded-tl-none flex items-center shadow-sm">
                  ...
                  <span className="text-xs font-medium">Analyzing your finances...</span>
                </div>
              </div>
            )}
          </div>
      ...
      <CardFooter className="p-6 pt-2 bg-card border-t border-border/50">
        <form onSubmit={handleAsk} className="flex w-full items-center gap-3">
          ...
          />
          <Button type="submit" size="icon" disabled={isLoading || !query.trim()} className="rounded-xl bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20">
