      ...
      <SidebarHeader className="p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="font-black text-lg tracking-tight leading-none">FinTrack</span>
            <span className="text-[10px] text-primary font-bold uppercase tracking-widest mt-0.5">Premium AI</span>
          </div>
        </div>
      </SidebarHeader>
      <SidebarContent className="px-3">
        <SidebarGroup>
          <SidebarGroupLabel className="px-4 text-[10px] uppercase tracking-widest font-bold">Main Menu</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu className="gap-1">
              {items.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={pathname === item.url} className="rounded-xl h-11 px-4 transition-all hover:bg-primary/5 data-[active=true]:bg-primary/10 data-[active=true]:text-primary">
                    <Link href={item.url}>
        ...
        <div className="mt-8 px-4 py-6 rounded-2xl bg-gradient-to-br from-primary/10 to-secondary/10 border border-primary/10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-black uppercase tracking-wider">AI Insights</span>
          </div>
          ...
          <SidebarMenuItem>
            <SidebarMenuButton className="w-full justify-start gap-3 p-2 rounded-xl h-14 hover:bg-muted/50 transition-all border border-transparent hover:border-border/50">
              <Avatar className="h-9 w-9 border-2 border-primary/20">
                ...
                <AvatarFallback className="bg-primary/10 text-primary font-bold">FM</AvatarFallback>
              </Avatar>
              <div className="flex flex-col items-start overflow-hidden">
                <span className="text-sm font-bold truncate">Financial Maven</span>
                <span className="text-[10px] font-bold text-primary uppercase tracking-tighter">Pro Member</span>
              </div>
            </SidebarMenuButton>
          </SidebarMenuItem>
